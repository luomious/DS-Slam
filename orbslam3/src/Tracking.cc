...
Sophus::SE3f Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp, string filename)
{
    ...
    if (mpSystem->GetVisualizer())
    {
        ...
        // Send dense point cloud from StaticMapper every N keyframes
#ifndef DS_SLAM_DISABLED
        static int denseSendCounter = 0;
        denseSendCounter++;
        if (denseSendCounter >= 5 && mpSystem->GetStaticMapper()) {
            denseSendCounter = 0;
            auto densePts = mpSystem->GetStaticMapper()->GetDensePointsSampled(5000);
            if (densePts.size() >= 3) {
                std::vector<float> denseCoords;
                denseCoords.reserve(densePts.size() * 3);
                for (const auto& pt : densePts) {
                    denseCoords.push_back(pt.x);
                    denseCoords.push_back(pt.y);
                    denseCoords.push_back(pt.z);
                }
                mpSystem->GetVisualizer()->SendDensePoints(denseCoords, mpSystem->GetStaticMapper()->GetPointCount());
            }
        }
#endif
    }
