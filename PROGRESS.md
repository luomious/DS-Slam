...
##### 步骤1：USB摄像头采集模块（1天）
- [ ] 创建`rgbd_camera.cc`主程序
- [ ] 使用OpenCV `cv::VideoCapture`采集RGB帧
- [ ] 支持RealSense SDK采集RGB+深度帧（可选）
- [ ] 帧率控制（30fps）和时间戳同步
- [ ] **验证标准**：能成功采集摄像头画面并显示

##### 步骤2：相机标定配置（0.5天）
- [ ] 创建`config/usb_camera.yaml`相机配置文件
- [ ] 支持手动输入内参（fx, fy, cx, cy）
- [ ] 支持畸变参数（k1, k2, p1, p2）
- [ ] 提供相机标定工具使用说明
- [ ] **验证标准**：配置文件能被SLAM系统正确加载

##### 步骤3：实时语义分割集成（1天）
- [ ] 复用现有`SemanticSegmentator`类
- [ ] 对每帧RGB图像进行人像分割
- [ ] 生成二值掩码并应用于深度图像
- [ ] **验证标准**：分割掩码正确，人像区域被过滤
...
- [ ] 支持PLY格式导出
- [ ] 支持相机轨迹保存（TUM格式）
- [ ] **验证标准**：导出的PLY文件能在MeshLab/CloudCompare中打开

##### 步骤6：启动脚本与文档（0.5天）
- [ ] 修改`start_system.bat`支持摄像头模式
- [ ] 添加`start_camera.bat`一键启动脚本
- [ ] 编写`CAMERA_GUIDE.md`使用文档
- [ ] **验证标准**：新用户能按文档在10分钟内完成配置并运行
